...
fn render_site_cli_page() -> Result<String> {
    let (command_docs, global_options) = collect_cli_docs()?;
    ...
    push_command_table(&mut lines, "## Commands", &command_docs);
    push_option_table(&mut lines, "## Global Options", &global_options);
    push_examples(&mut lines, "## Examples", true);
...
fn render_commands_block() -> Result<String> {
    ...
    push_command_table(&mut lines, "### Commands", &command_docs);
    push_option_table(&mut lines, "### Global Options", &global_options);
    push_examples(&mut lines, "### Quick Examples", false);

    Ok(lines.join("\n"))
}

fn collect_cli_docs() -> Result<(Vec<CommandDoc>, Vec<OptionDoc>)> {
    ...
    Ok((command_docs, global_options))
}

fn push_command_table(lines: &mut Vec<String>, heading: &str, command_docs: &[CommandDoc]) {
    lines.extend([
        heading.to_string(),
        String::new(),
...
fn push_option_table(lines: &mut Vec<String>, heading: &str, global_options: &[OptionDoc]) {
    lines.extend([
        String::new(),
        heading.to_string(),
        String::new(),
...
fn push_examples(lines: &mut Vec<String>, heading: &str, include_server: bool) {
    lines.extend([
        String::new(),
        heading.to_string(),
        String::new(),
    ...
    if include_server {
        lines.push("owui-lint server".to_string());
    }
...
fn render_rules_block() -> Result<String> {
    ...
    let order = [
        "OWUI", "OWT", "OWP", "OWF", "OWA", "OWE", "OWPL", "OWSEC", "OTHER",
    ];
    for group in order {
        let rows = grouped
            .get(group)
