...
Full documentation is published at <https://christestet.github.io/owui-lint/>.
...
- The opt-in **security profile** (`OWSEC` rules, `--security`) adds scope-aware checks — e.g. flagging dangerous calls that run at *import time* (module/`__init__` scope) versus only inside a method. It is detection by **call-site presence + scope**, not data-flow/taint analysis: it does not track whether a value reaches a sink.

## Architecture

- Native binary distribution (`owui-lint`)
- YAML config for lint rules (`config.yml` or `owui-lint.yml`)
- LSP server for editor integration (`owui-lint server`)
...
flowchart TD
    ...
    B -->|server| K["Language Server<br/>(server/)"]
...
| Option | Description |
|--------|-------------|
| `-c, --config <PATH>` | Path to config file (default lookup: config.yml, config.yaml, owui-lint.yml, owui-lint.yaml). |
...
| `--security` | Enable the opt-in security profile (OWSEC rules) for import-time-execution and trust checks. Off by default. |
...
The **security profile** (`OWSEC` rules) is off by default so existing runs are
unaffected. Enable it with top-level `security: true` or the `--security` flag. These
rules target import-time code execution and other reviewer/admin trust concerns — see
the [Security (`OWSEC`)](#rules) rule group below.
...
`owui-lint server` runs the language server over stdio for editor clients.
...
If the binary is not on your `PATH`, configure an absolute path or a
